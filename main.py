...
bot = Client(
    ...
    api_hash=API_HASH,
    bot_token=BOT_TOKEN)
auth_users = [7217632299,7764674199,7037764817,8175027727,5398544600]
...
async def upload(bot: Client, m: Message):
    ...
    if user is not None and user not in sudo_users:
        await m.reply("**CONTACT TO OWNER --@URS_LUCIFER**", quote=True)
        return
    else:
        editable = await m.reply_text('*➠ 𝐒𝐞𝐧𝐝 𝐌𝐞 𝐘𝐨𝐮𝐫 𝐓𝐗𝐓 𝐅𝐢𝐥𝐞 𝐢𝐧 𝐀 𝐏𝐫𝐨𝐩𝐞𝐫 𝐖𝐚𝐲 \n\n➠ TXT FORMAT : LINK : URL \n➠ 𝐌𝐨𝐝𝐢𝐟𝐢𝐞𝐝 𝐁𝐲:  @URS_LUCIFER*')
...
async def account_login(bot: Client, m: Message):
    user = m.from_user.id if m.from_user is not None else None
    if user is not None and user not in sudo_users:
        await m.reply("**CONTACT TO OWNER**", quote=True)
        return
    else:
